...
Object.assign(T.en, {
  ledeHome: 'Guess your way across the Paris Métro — lines, landmarks, termini, and the stations in between. Two ways to play.',
...
Object.assign(T.fr, {
  ledeHome: 'On devine son chemin dans le métro parisien : lignes, monuments, terminus et tout ce qu’il y a entre les deux. Deux façons de jouer.',
...
function handleMessage(event, p) {
  if (M.isHost) {
    if (event === 'hello') {
      if (!M.players.some(x => x.id === p.id)) {
        if (M.players.length >= MAX_PLAYERS) { M.net.send('full', { id: p.id }); return; }
        M.players.push({ id: p.id, name: String(p.name || '?').slice(0, 18), score: 0, correct: 0, host: false });
      }
      broadcastSync();
      renderLive();
...
function takeAnswer(p) {
  ...
  broadcastSync(); renderLive();
  if (M.answeredCount >= M.players.length) { clearTimeout(M.hostTimer); setTimeout(closeQuestion, 250); }
}
...
function renderLive() {
  ...
  if (M.phase === 'reveal' || M.phase === 'final') {
    ...
    el.innerHTML = `${notice}
      ...
      <div class="card">
        <div class="eyebrow" style="margin-bottom:6px">${M.phase === 'final' ? t('terminusLabel') : t('standings')}</div>
        ${board(true)}
      </div>
      ${M.phase === 'final'
        ? `<button class="primary" id="leaveBtn2">${t('leave')}</button>`
        : `<p class="dimtext center">${t('qOf')(M.qIndex, M.rounds)}</p>`}`;
    const lv = $('#leaveBtn2'); if (lv) lv.onclick = leaveRoom;
    return;
  }
